    ...
    with open(args.output_file, 'w') as f_out:
        for item in tqdm(input_data, desc=f"Processing {os.path.basename(args.input_file)}"):
            ...
            if images_list: # 确保列表不为空
                ...
                continue # 或者其他处理逻辑
            interleave_inputs_con = processor([prompt_con,'generate an image'], images=[images],padding=True, return_tensors="pt", vqmodel=model.model.vqmodel).to(model.device, torch.bfloat16)
            # interleave_inputs_con = processor([prompt_con]  , return_tensors="pt")
            # interleave_input_ids_con = interleave_inputs_con['input_ids'].to(model.device)
            
            # Unconditional prompt for CFG
            prompt_uncon = "generate images for thinking"
            ...
            kwargs_con = {
                "input_ids": interleave_inputs_con['input_ids'],
            ...
            kwargs_uncon = {
                "input_ids": interleave_inputs_uncon['input_ids'],
                # "image_latents": interleave_inputs_uncon['image_latents'],
                "cfg_scale": None,
