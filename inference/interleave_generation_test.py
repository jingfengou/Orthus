...
def main():
    ...
    with open(args.output_file, 'w') as f_out:
        for item in tqdm(input_data, desc=f"Processing {os.path.basename(args.input_file)}"):
            ...
            interleave_inputs_uncon = processor([prompt_uncon], images=[images],padding=True, return_tensors="pt", vqmodel=model.model.vqmodel).to(model.device, torch.bfloat16)
