...
class InterleaveSFTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # 屏蔽所有图片 token
        image_start_indices = (input_ids == boi_token_id).nonzero(as_tuple=True)[0]
        for start_idx in image_start_indices:
            end_idx = min(start_idx + image_token_len, sequence_length - 1)
            labels[0, (start_idx + 1):min(end_idx + 2, sequence_length)] = -100
