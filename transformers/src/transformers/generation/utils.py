...
class GenerationMixin:
    ...
    def generate_cfg(
        ...
        logits_processor_list: Optional[List[LogitsProcessorList]] = None,
        ...
        input_ids_list=[]
        image_latents_list=[]
        logits_processor_list_new=[]
        logits_warper_list=[]
        stopping_criteria_list=[]
        generation_config_list_new=[]
        synced_gpus_list=[]
        streamer_list=[]
        model_kwargs_list=[]

        for (generation_config, logits_processor, kwargs) in zip(generation_config_list, logits_processor_list, kwargs_list):
        ...
        #只保留了sample和greedy_search 若不是这两种模式则报错
            assert generation_mode in (GenerationMode.SAMPLE, GenerationMode.GREEDY_SEARCH)
            ...
            logits_processor_list_new.append(prepared_logits_processor)
            logits_warper_list.append(prepared_logits_warper)
            stopping_criteria_list.append(prepared_stopping_criteria)
            generation_config_list_new.append(generation_config)
            synced_gpus_list.append(synced_gpus)
        ...
        if generate_wo_quant == True:
            result = self._sample_orthus_cfg(
                ...
                logits_processor_list=logits_processor_list_new,
                logits_warper_list=logits_warper_list,
                stopping_criteria_list=stopping_criteria_list,
                generation_config_list=generation_config_list_new,
...
#while end
    ...
    ### added sample method for orthus
    def _sample_orthus(
        self,
        input_ids: torch.LongTensor,
