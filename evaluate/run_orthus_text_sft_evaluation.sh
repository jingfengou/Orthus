...
echo "--- CONFIGURATION ---"
# 设置环境变量和GPU数量
NUM_GPUS=7 # 您服务器上的GPU数量
export CUDA_VISIBLE_DEVICES=0,1,2,3,5,6,7 # 设置可见的GPU设备
# 定义项目和文件的路径
MODE="test" # 可选值：train, val, test
VERSION="v4"

ORTHUS_DIR="../"
BENCHMARK_DIR="/data1/oujingfeng/project/twgi/datasets/SpatialViz/datasets/${MODE}.jsonl" 

IMAGE_DIR="../../datasets/SpatialViz"    # 这里现在指向整个benchmark目录

# 定义临时和最终文件的名称
PREPROCESSED_FILE="orthus_${MODE}_input_for_sft.jsonl"
FINAL_INFERENCE_OUTPUT_FILE="orthus_output_sft_${MODE}_${VERSION}_final.jsonl"
CHUNK_DIR="temp_sft_${MODE}_chunks" # 存放数据分片的临时目录
PARTIAL_OUTPUT_DIR="temp_partial_${MODE}_${VERSION}_outputs" # 存放送部分结果的临时目录
# MODEL_CHECKPOINT="SJTU-Deng-Lab/Orthus-7B-instruct"
MODEL_CHECKPOINT="/data1/oujingfeng/project/twgi/checkpoints/orthus-7b-sft-think-${VERSION}"

LOG_DIR="logs_${MODE}_${VERSION}" # <-- 新增：日志目录
...
python preprocess_sft_benchmark.py \
    --input_file "$BENCHMARK_DIR" \
...
do
    ...
    CUDA_VISIBLE_DEVICES=$i python ../inference/interleave_generation.py \
...
echo
# FINAL_INFERENCE_OUTPUT_FILE="orthus_instruct_Question-instruct_output.jsonl" # <-- 新增：指定最终输出文件
# --- 6. 运行评估 ---
echo "--- [STEP 5/5] Evaluating the final results... ---"
python final_evaluate.py \
    --prediction_file "$FINAL_INFERENCE_OUTPUT_FILE" \
    --benchmark_dir "$BENCHMARK_DIR" \
    --output_results_file "final_evaluation_results_${MODE}_${VERSION}.json" # <-- 新增：指定输出文件
echo
